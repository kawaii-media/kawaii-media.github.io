...
export class RestTimer {
  ...
  private duration: number = 60; // default 60s
  private timeLeft: number = 60;
  private isRunning: boolean = false;
  private intervalId: number | null = null;
  private autoStart: boolean = false;
  private history: TimerLog[] = [];
  ...
  public getIsAutoStartEnabled(): boolean {
  ...
  private adjustTime(seconds: number) {
  ...
  private setPreset(seconds: number) {
    this.duration = seconds;
    this.timeLeft = seconds;
  ...
  private logTimer(duration: number, status: "Selesai" | "Dibatalkan") {
  ...
  private updateUI() {
    ...
    const playPauseBtn = this.container.querySelector("#timer-play-pause-btn") as HTMLButtonElement;
    if (playPauseBtn) {
    ...
    // Update timer text
    const timerDisplay = this.container.querySelector("#timer-countdown-display") as HTMLElement;
    if (timerDisplay) {
      timerDisplay.innerText = this.formatTime(this.timeLeft);
    }

    // Update SVG progress ring
    const ring = this.container.querySelector("#timer-progress-ring") as SVGPathElement;
    if (ring) {
  ...
  private renderHistory() {
    const list = this.container.querySelector("#timer-history-list") as HTMLElement;
    if (!list) return;
  ...
  private render() {
    ...
    const resetBtn = this.container.querySelector("#timer-reset-btn") as HTMLButtonElement;
    resetBtn.addEventListener("click", () => this.cancelTimer());

    const minusBtn = this.container.querySelector("#timer-minus-15-btn") as HTMLButtonElement;
    minusBtn.addEventListener("click", () => this.adjustTime(-15));

    const plusBtn = this.container.querySelector("#timer-plus-15-btn") as HTMLButtonElement;
    plusBtn.addEventListener("click", () => this.adjustTime(15));

    const testBtn = this.container.querySelector("#timer-sound-test-btn") as HTMLButtonElement;
    testBtn.addEventListener("click", () => playBeep());

    const autostartCheck = this.container.querySelector("#timer-autostart-checkbox") as HTMLInputElement;
    autostartCheck.addEventListener("change", (e) => {
      this.autoStart = (e.target as HTMLInputElement).checked;
    ...
    const clearHistoryBtn = this.container.querySelector("#clear-timer-history-btn") as HTMLButtonElement;
    if (clearHistoryBtn) {
