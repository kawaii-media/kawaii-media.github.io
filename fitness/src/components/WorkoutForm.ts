...
import { estimateCalories } from "../helpers";
...
export class WorkoutForm {
  ...
  private onAddWorkout: (workout: Omit<WorkoutEntry, "id">) => void;
  private onCancelEdit: () => void;

  private category: string = "Dada";
  private exerciseName: string = "Bench Press";
  private date: string = "";
  private notes: string = "";
  private sets: Omit<ExerciseSet, "id">[] = [{ reps: 10, weight: 0, completed: true }];
  private caloriesBurned: number = 0;
  private error: string = "";
  private showSuccess: boolean = false;
  private editingWorkout: WorkoutEntry | null = null;
  private lastEditId: string | null = null;

  constructor(options: WorkoutFormOptions) {
  ...
  public setEditingWorkout(entry: WorkoutEntry | null) {
  ...
  private triggerCalorieEstimation() {
    ...
    const calInput = this.container.querySelector("#workout-calories-input") as HTMLInputElement;
    if (calInput) {
      calInput.value = this.caloriesBurned.toString();
    }
  }

  private handleCategoryChange(newCat: string) {
  ...
  private handleExerciseChange(newName: string) {
  ...
  private removeSetRow(idx: number) {
  ...
  private updateSetField(idx: number, field: keyof ExerciseSet, value: any) {
  ...
  private showValidationError(msg: string) {
    this.error = msg;
    const errorBanner = this.container.querySelector("#form-error-banner") as HTMLElement;
    if (errorBanner) {
  ...
  private showSuccessNotification() {
    ...
    const successBanner = this.container.querySelector("#form-success-banner") as HTMLElement;
    if (successBanner) {
  ...
  private handleSubmit(e: Event) {
    ...
    // Reset if we were not editing (if editing, App.tsx will set editEntry to null which resets us)
    if (!this.editingWorkout) {
      this.notes = "";
      // Keep category and date for convenience of logging multiple things
      if (this.category === "Kardio") {
      ...
      // Update fields
      const notesInput = this.container.querySelector("#workout-notes-input") as HTMLInputElement;
  ...
  private renderExerciseSelect() {
    const parent = this.container.querySelector("#exercise-select-container") as HTMLElement;
    if (!parent) return;
    ...
    if (filtered.length > 0) {
      ...
      // Bind listener
      const select = parent.querySelector("#exercise-select") as HTMLSelectElement;
      select.addEventListener("change", (e) => {
        const val = (e.target as HTMLSelectElement).value;
        const customWrapper = parent.querySelector("#custom-exercise-input-wrapper") as HTMLElement;
        const customInput = parent.querySelector("#exercise-name-custom-input") as HTMLInputElement;
      ...
      customInput.addEventListener("input", (e) => {
        this.handleExerciseChange((e.target as HTMLInputElement).value);
      });
      ...
      const input = parent.querySelector("#exercise-name-input") as HTMLInputElement;
  ...
  private renderQuickSelectChips() {
    const parent = this.container.querySelector("#chips-container") as HTMLElement;
    if (!parent) return;
    ...
    parent.querySelectorAll(".quick-select-chip").forEach((chip) => {
      chip.addEventListener("click", (e) => {
        ...
        // Change category dropdown selection if different
        const catSelect = this.container.querySelector("#category-select") as HTMLSelectElement;
  ...
  private updateQuickSelectChipsSelection() {
    const chips = this.container.querySelectorAll(".quick-select-chip") as NodeListOf<HTMLButtonElement>;
    chips.forEach((chip) => {
  ...
  private renderSetsManager() {
    const parent = this.container.querySelector("#sets-manager-container") as HTMLElement;
    if (!parent) return;
    ...
    const addBtn = parent.querySelector("#add-set-row-btn") as HTMLButtonElement;
  ...
  private render() {
    ...
    const form = this.container.querySelector("#workout-logging-form") as HTMLFormElement;
    form.addEventListener("submit", (e) => this.handleSubmit(e));

    const cancelEditBtn = this.container.querySelector("#cancel-edit-btn") as HTMLButtonElement;
    if (cancelEditBtn) {
    ...
    categorySelect.addEventListener("change", (e) => {
      this.handleCategoryChange((e.target as HTMLSelectElement).value);
    });

    const dateInput = this.container.querySelector("#workout-date-input") as HTMLInputElement;
    dateInput.addEventListener("change", (e) => {
      this.date = (e.target as HTMLInputElement).value;
    });

    const caloriesInput = this.container.querySelector("#workout-calories-input") as HTMLInputElement;
    caloriesInput.addEventListener("input", (e) => {
      this.caloriesBurned = parseFloat((e.target as HTMLInputElement).value) || 0;
    });

    const recalcBtn = this.container.querySelector("#recalc-calories-btn") as HTMLButtonElement;
