...
import { calculateStreak, getWeeklyDistribution, getCategoryDistribution } from "../helpers.js";
...
export class StatsDashboard {
  ...
  renderWeeklyChart() {
    const parent = this.container.querySelector("#weekly-frequency-chart");
    if (!parent) return;
    ...
    const isZeroCheck = counts.every(c => c === 0);
  ...
  renderCategorySplit() {
    const parent = this.container.querySelector("#category-distribution-container");
    if (!parent) return;
